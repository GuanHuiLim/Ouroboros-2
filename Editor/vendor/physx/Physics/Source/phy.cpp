...
/*-----------------------------------------------------------------------------*/
    ...
    PhysicsObject PhysxWorld::duplicateObject(phy_uuid::UUID id) {
        ...
        if (all_objects.contains(id)) {
            ...
            // RIGID
            physicsNewObject.setRigidType(physxObject.rigidID);

            // SHAPE
            //physicsNewObject.setShape(physxObject.shape);
           
            if (physxObject.shape == shape::box) {
                PxBoxGeometry box = m_objects.at(index).m_shape->getGeometry().box();
                physicsNewObject.reAttachShape(physxObject.rigidID, box);
            ...
            else if (physxObject.shape == shape::sphere) {
                physicsNewObject.reAttachShape(physxObject.rigidID, m_objects.at(index).m_shape->getGeometry().sphere());
                //PxReal rad = m_objects.at(index).m_shape->getGeometry().sphere().radius;
                //physicsNewObject.setSphereProperty(rad);
            } 
            else if (physxObject.shape == shape::capsule) {
                physicsNewObject.reAttachShape(physxObject.rigidID, m_objects.at(index).m_shape->getGeometry().capsule());
            ...
            //else if (physxObject.shape == shape::plane) {
            ...
            // TRIGGER
            physicsNewObject.setTriggerShape(physxObject.trigger);

            // GRAVITY
            physicsNewObject.disableGravity(physxObject.gravity);

            // KINEMATIC
            physicsNewObject.enableKinematic(physxObject.kinematic);

            // COLLIDER
            physicsNewObject.enableCollider(physxObject.collider);
