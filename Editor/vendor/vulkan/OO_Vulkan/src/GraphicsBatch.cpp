...
void GraphicsBatch::GenerateTextGeometry()
{
		...
		for (const auto& token : tokens)
		{
			...
			for (const auto& c : token)
			{
				...
				for (size_t i = 0; i < quadVertexCount; i++)
				{
					...
					// we translate back to "pivot point" before rotating then translate back
					// s_Data.GlyphVertexBufferPtr->position = glm::vec4(ui.position,0.0f)+mdl_xform * (verts[i]-glm::vec4(ui.position,0.0f));
					// s_Data.GlyphVertexBufferPtr->colour = ui.colour;
					// s_Data.GlyphVertexBufferPtr->texCoord = textureCoords[i];
					// s_Data.GlyphVertexBufferPtr->TexIndex = textureIndex; // this glyph's textures
					// s_Data.GlyphVertexBufferPtr->EntityID = ui.entityID; // this glyph's textures
					// s_Data.GlyphVertexBufferPtr++;
				}
				// s_Data.GlyphIndexCount += 6;
