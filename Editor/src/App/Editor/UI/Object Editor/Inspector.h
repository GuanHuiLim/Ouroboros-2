	...
	for (rttr::property prop : type.get_properties())
	{
			...
			else if (prop_type.is_class())//nested
			{
				...
				if (end_edit)
				{
					...
					oo::CommandStackManager::AddCommand(new oo::Component_ActionCommand<Component>
						(pre_edited, value, prop, gameobject.GetInstanceID()));
					pre_edited.clear();//reset this variant
				}
				if (edited == true)
				{
					if (pre_edited.is_valid() == false)//if the variant is empty that means it is ready to hold data
						pre_edited = prop.get_value(component);
		...
		else
		{
			...
			if (end_edit)
			{
				//undo redo command
				oo::CommandStackManager::AddCommand(new oo::Component_ActionCommand<Component>
					(pre_edited, v, prop, gameobject.GetInstanceID()));
				pre_edited.clear();//reset this variant
			}
			if (set_value == true)
			{
