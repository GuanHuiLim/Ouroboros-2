...
namespace oo
{
    ...
    std::vector<ScriptFieldInfo> const ScriptClassInfo::GetScriptFieldInfoAll() const
    {
        ...
        ScriptEngine::ForEachClassField(_class, [&resultList, &sample](MonoClassField* field)
        {
                ...
                if (!fieldValue.IsNullType())
                {
                    ...
                    if (header != nullptr)
                    {
                        MonoClassField* headerNameField = mono_class_get_field_from_name(ScriptEngine::GetClass("ScriptCore", "Ouroboros", "HeaderAttribute"), "Name");
                        MonoString* str = nullptr;
                        mono_field_get_value(header, headerNameField, &str);
