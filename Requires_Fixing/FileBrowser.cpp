...
#include "Editor/Utility/FileSystemUtills.h"
#include "Editor/Utility/ImGuiManager.h"
#include "Editor/Utility/ImGui_ToggleButton.h"
...
#include "Editor/Events/FileEventsFunction.h"

FileBrowser::FileBrowser(const std::string& rootpath)
	:m_rootpath{rootpath},m_currentpath{rootpath}
{
	FileBehaviour(rootpath);
	s_Icons =
	{
		{ ".png", oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_PNG")->Get_IMTEXTURE_ID() },
		{ ".wav" ,oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_WAV")->Get_IMTEXTURE_ID() },
		{ ".mp3" ,oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_MP3")->Get_IMTEXTURE_ID() },
		{ ".prefab" ,oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_Prefab")->Get_IMTEXTURE_ID() },
		{ ".scn" ,oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_Engine_Icon_White")->Get_IMTEXTURE_ID() },
		{ ".anim" ,oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_AnimationClip_Icon_White")->Get_IMTEXTURE_ID() },
		{ ".controller" ,oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_AnimatorController_Icon_White")->Get_IMTEXTURE_ID() },
	};

	oo::EventManager::Subscribe<OpenFileEvent>(&OpenSceneEvent);
...
void FileBrowser::DetailedBrowser()
{
	...
	else
	{
		...
		icon_size = ImGuiManager::image_small.x;
	...
	ImGui::BeginTable("temp", m_zoomlevel, 0, { 0,0 }, item_width);
...
void FileBrowser::PopupOptions()
{
	if (ImGui::BeginMenu("Create"))
	{
		...
		{
			std::string fileTemp = (EditorProject::GetSceneFolder() + "/Scene");
		...
		{
			std::string fileTemp = (EditorProject::GetScriptFolder() + "/NewScript");
...
void FileBrowser::DirectoryBrowser()
{
	...
	ImGui::BeginGroup();
	ImGui::Image(oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_GenericFile")->Get_IMTEXTURE_ID(), ImGuiManager::image_small);
	ImGui::SameLine();
	if (ImGui::Selectable("Assets"))
		FileBehaviour(EditorProject::GetAssetDirectory());
	ImGui::EndGroup();

	ImGui::BeginGroup();
	ImGui::Image(oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_Prefab")->Get_IMTEXTURE_ID(), ImGuiManager::image_small);
	ImGui::SameLine();
	if (ImGui::Selectable("Prefab"))
		FileBehaviour(EditorProject::GetPrefabFolder());
	ImGui::EndGroup();

	ImGui::BeginGroup();
	ImGui::Image(oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_Engine_Icon_White")->Get_IMTEXTURE_ID(), ImGuiManager::image_small);
	ImGui::SameLine();
	if (ImGui::Selectable("Scene"))
		FileBehaviour(EditorProject::GetSceneFolder());
	ImGui::EndGroup();

	ImGui::BeginGroup();
	ImGui::Image(oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_CS_Icon")->Get_IMTEXTURE_ID(), ImGuiManager::image_small);
	ImGui::SameLine();
	if (ImGui::Selectable("Script"))
		FileBehaviour(EditorProject::GetScriptFolder());
	ImGui::EndGroup();
...
{
	if (ext.empty())
		return oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_Folder")->Get_IMTEXTURE_ID();

	auto iter = s_Icons.find(ext);
	if (iter == s_Icons.end())
		return oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_GenericFile")->Get_IMTEXTURE_ID();
...
void FileBrowser::RecursiveDirective(const std::filesystem::path& path)
{
	...
	for (std::filesystem::directory_entry entry : std::filesystem::directory_iterator(path))
	{
		...
		ImGui::Image(oo::AssetManager::GetInternalAsset<oo::Texture>("Ouroboros_Folder")->Get_IMTEXTURE_ID(), { 15,15 });
